...
class ContentEntitySpec extends WordSpec with Matchers with BeforeAndAfterAll {
  ...
  val driver = new PersistentEntityTestDriver(system, new ContentEntity, userId.toString)

  val userNotFound = UUID.randomUUID()
  val notFoundDrive = new PersistentEntityTestDriver(system, new ContentEntity, userNotFound.toString)
